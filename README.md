...
changes, update `width`/`height` in `lib/end-of-lease.ts` (`showcase`), `lib/commercial.ts`, `lib/deep-cleaning.ts`, `lib/office.ts`, `lib/airbnb.ts`, `lib/move-in.ts`, `lib/post-construction.ts`, `lib/spring-cleaning.ts` or `lib/window-cleaning.ts` (`images`).
